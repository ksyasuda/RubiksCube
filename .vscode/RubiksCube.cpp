...
void solver_debug(int cube[6][3][3])
{
	...
	if (duration_cast<milliseconds>(end2 - start2).count() < 1)
		...
		std::cout << "Total runtime = " << duration_cast<minutes>(end2 - start2).count() << " minutes && " << (duration_cast<seconds>(end2-start2).count() - (60 * duration_cast<minutes>(end2 - start2).count())) << " seconds" << std::endl;
	else if (duration_cast<minutes>(end2 - start2).count() > 60)
		std::cout << "Total runtime = " << duration_cast<hours>(end2 - start2).count() << " hours" << " && " << (duration_cast<minutes>(end2-start2).count() - (60 * duration_cast<hours>(end2 - start2).count())) << " minutes" << std::endl;
}
